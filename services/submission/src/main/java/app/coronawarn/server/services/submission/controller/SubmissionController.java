...
import app.coronawarn.server.services.submission.verification.OtpVerifier;
...
public class SubmissionController {
  ...
  SubmissionController(DiagnosisKeyService diagnosisKeyService, OtpVerifier otpVerifier,
   ...
   * @param otp          An otp for diagnosis verification.
  ...
  public DeferredResult<ResponseEntity<Void>> submitDiagnosisKey(
      ...
      @RequestHeader("cwa-authorization") String otp) {
    submissionMonitor.incrementRequestCounter();
    submissionMonitor.incrementRealRequestCounter();
    return buildRealDeferredResult(exposureKeys, otp);
  }
  ...
  private DeferredResult<ResponseEntity<Void>> buildRealDeferredResult(SubmissionPayload submissionPayload,
    ...
    stopWatch.start();
    try {
      if (!this.otpVerifier.verifyOtp(otp)) {
